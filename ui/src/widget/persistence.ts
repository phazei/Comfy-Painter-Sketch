 ...
 *
 * Save: each dirty layer is encoded as PNG (exactly `bounds` sized, straight
 * alpha -- what `canvas.toBlob` produces), named by content hash and uploaded
 * with `POST /upload/image` (`type=input`, `subfolder=painter-sketch`,
 * `overwrite=true`: same name == same bytes). Unchanged content maps to the
 * same name and is not re-uploaded. A layer's `file` is only updated after a
 * successful upload, so the manifest never references a missing file; on
 * failure the layer stays dirty and the pixels stay in memory.
 *
 * Restore: layer PNGs are loaded from `/view` into the layer canvases; loads
 * for a session that was released meanwhile are ignored.
 */
...
/** Delay after the last edit before auto-uploading. */
export const AUTO_UPLOAD_DELAY_MS = 1000;
...
export class LayerUploader {
  ...
  private async uploadLayer(layerId: string, currentFile: string | null): Promise<string | null> {
...
/**
 * POST a PNG to `/upload/image`.
 * @returns `"painter-sketch/<name>.png [input]"` as reported by the server.
 */
...
/**
 * Load every layer's PNG into the editor. Painting is disabled until done.
 *
